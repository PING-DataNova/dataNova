...
def load_test_company_processes():
    """Charger les données entreprise (company_processes) depuis data/company_process/"""
    print("\n📦 Chargement des données entreprise (processes)...")
    
    session = get_session()
    repo = CompanyProcessRepository(session)
    
    # Dossier des données entreprise
    processes_dir = os.path.join(
        os.path.dirname(__file__),
        "../data/company_process"
    )
    
    if not os.path.exists(processes_dir):
        print("⚠️  Aucun dossier data/company_process trouvé")
        session.close()
        return
    ...
    for filename in os.listdir(processes_dir):
        ...
        try:
            ...
            repo.save(process)
            session.commit()
            print(f"✅ {company_name} - données process créées")
...
def main():
    ...
    # Charger les profils entreprise (optionnel)
    load_test = input("\n📦 Charger les profils entreprise de test ? (o/n): ")
    if load_test.lower() in ['o', 'y', 'oui', 'yes']:
        load_test_company_profiles()
    
    # Charger les données process (optionnel)
    load_processes = input("\n📦 Charger les données entreprise (processes) ? (o/n): ")
